...
//|       Black Tide Wave System — Institutional Context Engine v7.947|
...
input bool     InpShowSessionLines   = true;  // v7.943: draw a vertical dashed line at every session transition (for Test 1 verification)
input bool     InpAutoDST            = false; // v7.943: auto-adjust London/New York session hours for UK/US Daylight Saving Time (off by default — preserves existing fixed-hour behavior unless explicitly enabled)
...
struct MarketProfileEngineResult
{
   ...
   // Prior Session Profile History (Renamed from Yesterday for Exact Semantics)
   double PriorSessionPOC;
   double PriorSessionVAH;
   double PriorSessionVAL;
   double YesterdayHigh;
   double YesterdayLow;
   bool   HasPriorSessionProfile;
   ...
   {
      VAH = 0; VAL = 0; POC = 0;
      PriorSessionPOC = 0; PriorSessionVAH = 0; PriorSessionVAL = 0; YesterdayHigh = 0; YesterdayLow = 0;
      HasPriorSessionProfile = false;
...
void ResetSession();
void CheckFilters(int i,const double &o[],const double &h[],const double &l[],const double &c[],const long &tv[], double atrValue);
void CalcDoP(double cp);
// v7.943: auto-DST helpers (opt-in via InpAutoDST). Pure date math — no side effects,
// no interaction with session/profile/state/score data.
datetime GetNthSundayOfMonth(int year, int month, int n);
datetime GetLastSundayOfMonth(int year, int month);
bool     IsUKDST(datetime utcTime);
...
public:
   ...
   datetime CalculateSessionStart(datetime barTime, string &outSessionName)
   {
      ...
      // v7.943: Auto-DST (opt-in). g_SessionDef hours are the standard-time (winter)
      // baseline. When London is on BST or New York is on EDT, their real local session
      // open lands 1 hour earlier in UTC/server terms, so shift only the affected
      // session's start hour back by 1 for that bar's own calendar date — every other
      // calculation (asiaStart/overStart, the AUTO/LONDON/NEWYORK/TOKYO branches below,
      // session boundary detection) is untouched.
      if(InpAutoDST)
      {
         datetime approxUTC = barTime - g_svr_off * 3600;
         if(IsUKDST(approxUTC)) lonStart = (lonStart - 1 + 24) % 24;
         if(IsUSDST(approxUTC)) nyStart  = (nyStart  - 1 + 24) % 24;
      }
...
private:
   ...
   // --- v7.941 Phase A: incremental M1 replay cache (developing/replay path only). ---
   // The finalization (session-close) path above is untouched and does not use these.
   MqlRates               m_sessionCacheRates[];      // M1 bars for the current developing session, ascending (index 0 = oldest)
   int                    m_sessionCacheCount;        // number of valid bars in m_sessionCacheRates
   datetime               m_sessionCacheSessionStart; // which session m_sessionCacheRates belongs to
   bool                   m_cacheRangeInit;           // whether m_cachedHighest/m_cachedLowest have been seeded yet for this cache
   ...
   void CalculateProfileHistory(MarketProfileEngineResult &outData, datetime targetTime)
   {
      // =====================================================================================
      // v7.94-7 STEP 2 (Phase A — correctness only, no caching yet):
      // Historical D1/W1 context must be resolved relative to targetTime, not always
      // "yesterday/last week relative to the machine's current time".
      //
      // Historical replay (targetTime > 0):
      //     targetTime -> D1/W1 bar CONTAINING targetTime -> +1 -> previous COMPLETED D1/W1 bar
      // Live/finalization fallback (targetTime == 0):
      //     preserve the original shift=1 behavior exactly.
      //
      // Profile mathematics, POC/VAH/VAL, Adaptive Bin, Developing Profile, Prior Session
      // logic below are untouched — only WHERE d1Rates[0]/w1Rates[0] come from changed.
      // =====================================================================================
      int d1Shift = 1;
      if(targetTime > 0)
      {
         int targetD1Shift = iBarShift(m_symbol, PERIOD_D1, targetTime, false);
         if(targetD1Shift >= 0) d1Shift = targetD1Shift + 1;
      }

      MqlRates d1Rates[];
      ArraySetAsSeries(d1Rates, true);
      if(CopyRates(m_symbol, PERIOD_D1, d1Shift, 1, d1Rates) > 0)
      {
         ...
         if(g_pri.poc > 0)
         {
            outData.PriorSessionPOC        = g_pri.poc;
            outData.PriorSessionVAH        = g_pri.vah;
            outData.PriorSessionVAL        = g_pri.val;
            outData.HasPriorSessionProfile = true;
         }
         else
         {
            outData.PriorSessionPOC        = (d1Rates[0].high + d1Rates[0].low + d1Rates[0].close) / 3.0; // Prior Session Typical Price
            outData.PriorSessionVAH        = d1Rates[0].high;                                              
            outData.PriorSessionVAL        = d1Rates[0].low;                                               
            outData.HasPriorSessionProfile = false;
         }
      }
      ...
      if(targetTime > 0)
      {
         int targetW1Shift = iBarShift(m_symbol, PERIOD_W1, targetTime, false);
         if(targetW1Shift >= 0) w1Shift = targetW1Shift + 1;
      }

      MqlRates w1Rates[];
      ArraySetAsSeries(w1Rates, true);
      if(CopyRates(m_symbol, PERIOD_W1, w1Shift, 1, w1Rates) > 0)
      {
   ...
   // Distributes one M1 bar's volume across its low..high bins using the SAME
   // formula as the original full-rebuild loop (unchanged math, v7.941 Phase A
   // just calls this from more than one place so the formula is defined once).
   void DistributeBarVolumeIntoBins(double barLow, double barHigh, double barVol)
   {
      int lowBin  = (int)((barLow  - m_cachedLowest) / m_cachedProfileBinSize);
      int highBin = (int)((barHigh - m_cachedLowest) / m_cachedProfileBinSize);
   ...
   // v7.94-2 BUG FIX: on a genuine session change, ALL profile state derived from the
   // previous session must be invalidated together — not just the M1 replay cache.
   // Previously only m_sessionCacheRates/m_sessionCacheCount were cleared, leaving
   // m_cachedHighest/m_cachedLowest/m_cachedPriceBins/m_volArray/m_cachedTotalVolume
   // from the OLD session in place. If the new session's range happened to fall
   // entirely inside the old session's range, needFullRebuild's
   // (newHighest>m_cachedHighest || newLowest<m_cachedLowest) check would stay false,
   // so the new session would silently keep using the OLD session's bins/volume —
   // cross-session profile contamination. This resets every piece of that state
   // together, once, in one place, without touching the Adaptive Bin / Volume
   // Distribution / POC / VA algorithms themselves.
   void ResetProfileStateForNewSession(datetime newSessionStart)
   {
      ArrayFree(m_sessionCacheRates);
      m_sessionCacheCount        = 0;
      m_sessionCacheSessionStart = newSessionStart;
      m_cacheRangeInit           = false;
...
public:
   CMarketProfileEngine()
   {
      ...
      m_sessionCacheCount        = 0;
      m_sessionCacheSessionStart = 0;
      m_cacheRangeInit           = false;
      ArrayFree(m_sessionCacheRates);
   }
   
   void Update(const SessionEngineResult &sessionInput, MarketProfileEngineResult &outData, double currentAtr = 0.0, datetime sessionEnd = 0, double analysisPrice = 0.0, datetime targetTime = 0)
   {
      if(!InpEnableMP) return;
      outData.Reset();
      
      CalculateProfileHistory(outData, targetTime);
      ...
      {
         // =====================================================================================
         // FINALIZATION PATH (session-close snapshot) — v7.94 logic, unchanged. Called once per
         // session transition (not per bar), so its one-off CopyRates/iHighest/iLowest cost is
         // unaffected by the v7.941 Phase A replay-cache work in the "else" branch below.
         // =====================================================================================
         int endBar = iBarShift(m_symbol, PERIOD_M1, sessionEnd);
         ...
         for(int i = totalBars - 1; i >= 0; i--)
         {
            ...
            DistributeBarVolumeIntoBins(rates[i].low, rates[i].high, barVol);
         ...
         // The session that just finalized is done; its developing cache (if any) is now
         // stale and will be rebuilt fresh the next time a developing call arrives.
         ArrayFree(m_sessionCacheRates);
         m_sessionCacheCount        = 0;
         m_sessionCacheSessionStart = 0;
         m_cacheRangeInit           = false;
      ...
      {
         // =====================================================================================
         // DEVELOPING / REPLAY PATH — v7.941 Phase A.
         // Same POC/VAH/VAL/IB/Adaptive-Bin math as always (DistributeBarVolumeIntoBins is the
         // exact formula the original full-rebuild loop used). What changed is ONLY how M1 data
         // is acquired: instead of CopyRates(M1, whole session)+iHighest+iLowest on every single
         // call (the cause of the freeze/missing-arrow regression when called once per bar), M1
         // bars are fetched once per session and cached, extended incrementally as replay moves
         // forward, with running high/low tracked instead of repeated iHighest/iLowest scans.
         // 'targetTime' (new, optional) lets the caller replay a HISTORICAL bar's own time
         // instead of always "now" — sessionEnd==0 previously meant "up to now" unconditionally,
         // which is only correct for the live/current bar, not for a bar deep in history.
         // =====================================================================================
         int endBar = (targetTime > 0) ? iBarShift(m_symbol, PERIOD_M1, targetTime) : 0;
         if(endBar < 0) endBar = 0;

         if(sessionInput.sessionStart != m_sessionCacheSessionStart)
         {
            ResetProfileStateForNewSession(sessionInput.sessionStart);
         }

         double newHighest    = m_cachedHighest;
         double newLowest     = m_cachedLowest;
         bool   touchedAnyBar = false;
         int    appendedCount = 0;
         bool   refreshedLast = false;
         double refreshDelta  = 0.0;

         if(m_sessionCacheCount == 0)
         {
            // First fetch for this session: pull the whole session-to-date range ONCE
            // (same one-off cost the original code paid on every call — now paid only
            // once per session instead of once per chart bar).
            int totalBars = startBar - endBar + 1;
            if(totalBars <= 0) return;

            MqlRates freshRates[];
            ArraySetAsSeries(freshRates, true);
            if(CopyRates(m_symbol, PERIOD_M1, endBar, totalBars, freshRates) <= 0) return;

            ArrayResize(m_sessionCacheRates, totalBars);
            for(int k = 0; k < totalBars; k++)
            {
               m_sessionCacheRates[k] = freshRates[totalBars - 1 - k]; // reverse into ascending (oldest..newest)
               double bh = m_sessionCacheRates[k].high;
               double bl = m_sessionCacheRates[k].low;
               if(!m_cacheRangeInit) { newHighest = bh; newLowest = bl; m_cacheRangeInit = true; }
               else { newHighest = MathMax(newHighest, bh); newLowest = MathMin(newLowest, bl); }
            }
            m_sessionCacheCount = totalBars;
            touchedAnyBar = true;
         }
         else
         {
            datetime newestCachedTime  = m_sessionCacheRates[m_sessionCacheCount - 1].time;
            int      newestCachedShift = iBarShift(m_symbol, PERIOD_M1, newestCachedTime);

            if(newestCachedShift > endBar)
            {
               // New M1 bars have closed since the last call: fetch only the increment
               // (typically the handful of M1 bars spanned by one chart bar) and append —
               // never re-fetch bars already cached.
               int extendCount = newestCachedShift - endBar;
               MqlRates newRates[];
               ArraySetAsSeries(newRates, true);
               if(CopyRates(m_symbol, PERIOD_M1, endBar, extendCount, newRates) > 0)
               {
                  int oldCount = m_sessionCacheCount;
                  ArrayResize(m_sessionCacheRates, oldCount + extendCount);
                  for(int k = 0; k < extendCount; k++)
                  {
                     m_sessionCacheRates[oldCount + k] = newRates[extendCount - 1 - k];
                     double bh = m_sessionCacheRates[oldCount + k].high;
                     double bl = m_sessionCacheRates[oldCount + k].low;
                     if(!m_cacheRangeInit) { newHighest = bh; newLowest = bl; m_cacheRangeInit = true; }
                     else { newHighest = MathMax(newHighest, bh); newLowest = MathMin(newLowest, bl); }
                  }
                  m_sessionCacheCount = oldCount + extendCount;
                  appendedCount = extendCount;
                  touchedAnyBar = true;
               }
            }
            else if(newestCachedShift == endBar)
            {
               // Same M1 bar as last call — it may still be forming live: refresh just that
               // one bar so intrabar volume growth isn't missed, without re-scanning anything else.
               MqlRates refreshed[];
               ArraySetAsSeries(refreshed, true);
               if(CopyRates(m_symbol, PERIOD_M1, endBar, 1, refreshed) > 0 && refreshed[0].time == newestCachedTime)
               {
                  double oldVol = (double)(m_sessionCacheRates[m_sessionCacheCount-1].real_volume > 0 ? m_sessionCacheRates[m_sessionCacheCount-1].real_volume : m_sessionCacheRates[m_sessionCacheCount-1].tick_volume);
                  double newVol = (double)(refreshed[0].real_volume > 0 ? refreshed[0].real_volume : refreshed[0].tick_volume);
                  m_sessionCacheRates[m_sessionCacheCount - 1] = refreshed[0];
                  double bh = refreshed[0].high;
                  double bl = refreshed[0].low;
                  if(!m_cacheRangeInit) { newHighest = bh; newLowest = bl; m_cacheRangeInit = true; }
                  else { newHighest = MathMax(newHighest, bh); newLowest = MathMin(newLowest, bl); }
                  if(newVol > oldVol) { refreshedLast = true; refreshDelta = newVol - oldVol; touchedAnyBar = true; }
               }
            }
            // else: target is behind what's already cached (shouldn't happen in forward
            // replay) — reuse the existing cache/bins as-is.
         }

         if(!touchedAnyBar && m_cachedPriceBins <= 0) return; // nothing usable yet

         bool needFullRebuild = (m_cachedPriceBins <= 0) ||
                                (newHighest > m_cachedHighest) ||
                                (newLowest  < m_cachedLowest);

         if(needFullRebuild)
         {
            m_lastSessionStart = sessionInput.sessionStart;
            m_cachedHighest    = newHighest;
            m_cachedLowest     = newLowest;
            ...
            int ticksPerBin = MathMax(InpMinBinTicks, InpProfileBinTicks);
            double minBinSize = tickSize * ticksPerBin;
            double range = m_cachedHighest - m_cachedLowest;
            if(range <= 0) return;
            ...
            // Rebuild entirely from the CACHED M1 array (in-memory — no CopyRates/iHighest/
            // iLowest here). This is the "rebin using the already-cached array" step, replacing
            // what used to be a fresh terminal-history scan on every single call.
            for(int i = 0; i < m_sessionCacheCount; i++)
            {
               double barVol = (double)(m_sessionCacheRates[i].real_volume > 0 ? m_sessionCacheRates[i].real_volume : m_sessionCacheRates[i].tick_volume);
               m_cachedTotalVolume += barVol;

               DistributeBarVolumeIntoBins(m_sessionCacheRates[i].low, m_sessionCacheRates[i].high, barVol);

               MqlDateTime barDt;
               TimeToStruct(m_sessionCacheRates[i].time, barDt);
               if((barDt.min == 0 || barDt.min == 30) && m_sessionCacheRates[i].time != m_lastSampleTime)
               {
                  UpdateDevelopingProfileNode(m_sessionCacheRates[i].time, outData);
               }
            }
         }
         else if(appendedCount > 0)
         {
            // No new extreme: bin boundaries stay valid, so just add the volume of the bar(s)
            // newly appended this call (ALL of them, not just the latest one — this also closes
            // a latent gap where charts above M1 used to credit only the newest M1 bar's volume
            // and silently skip the bars in between on higher timeframes).
            int startIdx = m_sessionCacheCount - appendedCount;
            for(int i = startIdx; i < m_sessionCacheCount; i++)
            {
               double barVol = (double)(m_sessionCacheRates[i].real_volume > 0 ? m_sessionCacheRates[i].real_volume : m_sessionCacheRates[i].tick_volume);
               m_cachedTotalVolume += barVol;
               DistributeBarVolumeIntoBins(m_sessionCacheRates[i].low, m_sessionCacheRates[i].high, barVol);
         ...
         else if(refreshedLast)
         {
            int lastIdx = m_sessionCacheCount - 1;
            m_cachedTotalVolume += refreshDelta;
            DistributeBarVolumeIntoBins(m_sessionCacheRates[lastIdx].low, m_sessionCacheRates[lastIdx].high, refreshDelta);
         }
      }
      ...
      // Avoid current BID look-ahead contamination during historical analysis
      double evalPrice = (analysisPrice > 0.0) ? analysisPrice : SymbolInfoDouble(m_symbol, SYMBOL_BID);
      outData.PriceAboveVAH = (evalPrice > outData.VAH);
      outData.PriceBelowVAL = (evalPrice < outData.VAL);
      outData.PriceInsideVA = (evalPrice >= outData.VAL && evalPrice <= outData.VAH);
      outData.PriceAbovePOC = (evalPrice > outData.POC);
      outData.PriceBelowPOC = (evalPrice < outData.POC);
      outData.PriceInsideIB  = (evalPrice >= sessionInput.ibLow && evalPrice <= sessionInput.ibHigh);
      outData.PriceAboveIBH = (evalPrice > sessionInput.ibHigh);
      outData.PriceBelowIBL = (evalPrice < sessionInput.ibLow);
...
public:
   ...
   void AnalyzeState(const MarketProfileEngineResult &mp, const double closePrice, const double highPrice, const double lowPrice, double atr, ENUM_IB_EXPLICIT_STATE sessionIbState, datetime barTime, MarketStateEngineResult &outState)
   {
      outState.Reset();

      // Wire CSessionEngine IB lifecycle into MarketStateEngine (v7.941 Priority 2 Fix)
      // Only classify a break once the Initial Balance is fully formed (IB_COMPLETE),
      // reusing mp.PriceAboveIBH / mp.PriceBelowIBL which are already computed against
      // the correct session IB range and evaluation price.
      if(sessionIbState == IB_COMPLETE)
      {
         if(mp.PriceAboveIBH)      outState.ibState = IB_BREAK_ABOVE;
         else if(mp.PriceBelowIBL) outState.ibState = IB_BREAK_BELOW;
         else                      outState.ibState = IB_INSIDE;
      }
      else
      {
         outState.ibState = IB_INSIDE;
      }
      ...
      // v7.94-2 review item 11: iClose(HTF, 1) always meant "previous closed HTF bar
      // relative to NOW", so a historical bar was silently evaluated against TODAY's
      // HTF close. Resolving the HTF bar via iBarShift(HTF, barTime) first generalizes
      // the same "shift 1 back" semantics to bar i's own time: for barTime == now this
      // reduces to shift 1, identical to the original.
      int htf1Shift = iBarShift(m_symbol, InpStateHTF1, barTime);
      if(htf1Shift < 0) htf1Shift = 0;
      int htf2Shift = iBarShift(m_symbol, InpStateHTF2, barTime);
      if(htf2Shift < 0) htf2Shift = 0;

      double htf1Close = iClose(m_symbol, InpStateHTF1, htf1Shift + 1);
      double htf2Close = iClose(m_symbol, InpStateHTF2, htf2Shift + 1);
...
public:
   ...
   void EvaluatePhase5Confluence(int idx, const Phase4ContextData &p4, const MarketStateEngineResult &p2, const double &atr, datetime barTime, Phase5ConfluenceData &outP5)
   {
      outP5.Reset();
      ...
      for(int k = 2; k < 5; k++) 
      {
         ...
         {
            // v7.94-2 review item 11: shift 0 always meant "the current, still-forming
            // HTF bar right now" — for a historical bar this pulled in market state from
            // AFTER that bar's own time. Resolve the HTF bar shift relative to barTime.
            int mtfShift = iBarShift(_Symbol, g_mtfT[k], barTime);
            if(mtfShift < 0) mtfShift = 0;
            if(CopyBuffer(g_hMTF_Fast[k], 0, mtfShift, 1, ef) > 0 && CopyBuffer(g_hMTF_Slow[k], 0, mtfShift, 1, es) > 0)
            {
...
bool DetectDisplacement(int i, const double &o[], const double &h[], const double &l[], const double &c[], bool isBullish, double atrValue);
...
bool DetectInstitutionalFVG(int i, const double &o[], const double &h[], const double &l[], const double &c[], bool isBullish, double atrValue);
void UpdateOrderBlock(int i, const double &o[], const double &h[], const double &l[], const double &c[]);
void UpdateLiquidityMap(int i, const double &h[], const double &l[], const datetime &t[], double atrValue);
...
double CalculateInstitutionalScore(int i, const double &o[], const double &h[], const double &l[], const double &c[], const long &tv[], bool isBullish, datetime barTime, double atrValue);
double GetDynamicPercentileThreshold(int currentIndex);

void CheckSignals(int i, const double &o[], const double &h[], const double &l[], const double &c[], const long &tv[], const datetime &t[], bool isHistory, double atrValue);
void DrawSig(int i,double p,double h,double l,int type,const datetime &t[], double score, bool isHistory, double atrValue);
void DrawSessionBoundary(datetime transitionTime, string sessionName); // v7.943: Test 1 visual verification aid
...
int OnInit()
{
   ...
   IndicatorSetString(INDICATOR_SHORTNAME,"BTW v7.947 | "+TFName()+"|"+g_broker);
...
//--- Event Handler for Interactive Arrow Click
                ...
                const long &tv[], const long &v[], const int &sp[])
{
   ...
   // ============================================================================
   // v7.94-5 — P0 diagnostic, part 1 (temporary, user-requested verification only).
   // Confirms empirically whether t[]/o[]/h[]/l[]/c[]/tv[]/v[] are indexed as a
   // timeseries (index 0 = newest/current bar) or as chronological/non-series
   // (index 0 = oldest bar), which the historical replay loop below assumes.
   // Fires ONCE on the initial full calculation only (pc==0) and never again on
   // subsequent tick updates. Pure Print()/PrintFormat() calls — no array is
   // touched, resized, or re-flagged, and no existing logic is altered.
   // Safe to delete this block once you've confirmed the direction in your log.
   // ============================================================================
   if(pc == 0 && rt > 3)
   {
      PrintFormat("[v7.94-5 DIAG] ArrayGetAsSeries -> t=%s o=%s h=%s l=%s c=%s tv=%s v=%s",
                  (string)ArrayGetAsSeries(t), (string)ArrayGetAsSeries(o), (string)ArrayGetAsSeries(h),
                  (string)ArrayGetAsSeries(l), (string)ArrayGetAsSeries(c), (string)ArrayGetAsSeries(tv),
                  (string)ArrayGetAsSeries(v));
      PrintFormat("[v7.94-5 DIAG] rates_total=%d prev_calculated=%d | t[0]=%s  t[1]=%s  t[2]=%s  ...  t[rt-3]=%s  t[rt-2]=%s  t[rt-1]=%s",
                  rt, pc,
                  TimeToString(t[0], TIME_DATE|TIME_MINUTES), TimeToString(t[1], TIME_DATE|TIME_MINUTES), TimeToString(t[2], TIME_DATE|TIME_MINUTES),
                  TimeToString(t[rt-3], TIME_DATE|TIME_MINUTES), TimeToString(t[rt-2], TIME_DATE|TIME_MINUTES), TimeToString(t[rt-1], TIME_DATE|TIME_MINUTES));
      Print("[v7.94-5 DIAG] Read this as: if t[0] is the OLDEST time and t[rt-1] is the NEWEST (closest to now), ",
            "the 'for(i=s; i<rt; i++)' replay loop below walks PAST -> PRESENT correctly (chronological, non-series). ",
            "If instead t[0] is the NEWEST and t[rt-1] is the OLDEST, indexing is reversed (timeseries) and the loop direction must be fixed before anything else.");
   }
   // ============================================================================
   // End v7.94-5 P0 diagnostic block (part 1).
   // ============================================================================
   ...
   if(pc == 0) 
   {
      ...
      // v7.94-6 fix: OnCalculate() can be re-invoked with pc==0 (a full recalculation)
      // WITHOUT OnInit() running again (e.g. history refresh/reconnect), in which case
      // these globals would otherwise still hold values left over from whatever the
      // previous calculation's LAST bar was — letting the first historical bars of the
      // new replay pass see market-structure/order-block/liquidity/prior-session state
      // that shouldn't exist yet at that point in history. Reset them here so every
      // full replay starts from the same clean slate as a fresh OnInit().
      g_marketState = STATE_TRANSITION;
      g_activeOB.valid     = false;
   ...
   // v7.94-5 P0 fix: explicitly force these CopyBuffer destination arrays to
   // non-series (index 0 = oldest) BEFORE copying, so their ordering matches
   // t[]/o[]/h[]/l[]/c[]/tv[]/v[] by explicit guarantee rather than by relying on
   // whatever MQL5's default happens to be for a freshly declared dynamic array.
   // This does not change any value — CopyBuffer(handle, buffer, 0, rt, arr) with
   // start_pos=0 still requests "the most recent rt bars", it only pins down HOW
   // those bars land in the array (ascending, oldest-first) with certainty.
   ArraySetAsSeries(g_f,  false);
   ArraySetAsSeries(g_sl, false);
   ArraySetAsSeries(g_e2, false);
   ...
   // Per-bar historical ATR (v7.941 Priority 1/3 Fix): fetched with the same
   // call pattern as the EMA buffers above, so atrFastArr[i] aligns 1:1 with
   // bar i (index 0 = oldest, matching t[]/o[]/h[]/l[]/c[]). This lets every
   // engine below evaluate bar i against the ATR that existed AT bar i,
   // instead of the current-only g_atr_fast scalar.
   double atrFastArr[];
   ArraySetAsSeries(atrFastArr, false); // v7.94-5 P0 fix: same explicit normalization as above
   if(CopyBuffer(g_hATR_Fast, 0, 0, rt, atrFastArr) <= 0) return 0;

   // ============================================================================
   // v7.94-5 — P0 diagnostic, part 2 (temporary, user-requested verification only).
   // Part 1 (above, near 's') already confirmed t[]/o[]/h[]/l[]/c[]/tv[]/v[] are
   // non-series (index 0 = oldest). This confirms the CopyBuffer-sourced arrays
   // (g_f/g_sl/g_e2/atrFastArr) — which now have an EXPLICIT ArraySetAsSeries(...,
   // false) immediately before their CopyBuffer call — land in the exact same
   // ordering and size as t[]/c[], so g_f[i]/atrFastArr[i] genuinely refer to the
   // same bar i as t[i]/c[i], not a different bar via a CopyBuffer-specific default.
   // Pure Print()/ArrayGetAsSeries()/ArraySize() calls — no value is changed.
   // ============================================================================
   if(pc == 0 && rt > 3)
   {
      PrintFormat("[v7.94-5 DIAG] ArrayGetAsSeries -> g_f=%s g_sl=%s g_e2=%s atrFastArr=%s  |  ArraySize -> t=%d g_f=%d g_sl=%d g_e2=%d atrFastArr=%d",
                  (string)ArrayGetAsSeries(g_f), (string)ArrayGetAsSeries(g_sl), (string)ArrayGetAsSeries(g_e2), (string)ArrayGetAsSeries(atrFastArr),
                  ArraySize(t), ArraySize(g_f), ArraySize(g_sl), ArraySize(g_e2), ArraySize(atrFastArr));
      Print("[v7.94-5 DIAG] Read this as: ArrayGetAsSeries must read 'false' for all four, and every ArraySize must equal rt. ",
            "That combination guarantees g_f[i]/g_sl[i]/g_e2[i]/atrFastArr[i] are the SAME bar i as t[i]/c[i] for every i, ",
            "closing the CopyBuffer-alignment concern (not just the t[]/c[] direction confirmed in part 1 above).");
   }
   // ============================================================================
   // End v7.94-5 P0 diagnostic block.
   // ============================================================================


   // Determine Effective IB duration (Auto TF vs Manual Input Override)
   int effectiveIB = InpAutoTF ? g_ib : InpIBMinutes;
   ...
   for(int i=s; i<rt; i++)
   {
      ...
      if(g_SessionEngine.GetSessionStart() != 0 && barSessionStart != g_SessionEngine.GetSessionStart())
      {
         ...
         g_MPEngine.Update(g_SessionRes, g_ProfileRes, atrFastArr[i], barSessionStart - 1, c[i]);
         SavePrior();
         ResetSession();
         g_SessionEngine.OnNewSession(barSessionStart, barSessionName);
         DrawSessionBoundary(barSessionStart, barSessionName);
      }
      ...
      g_SessionEngine.ProcessBar(t[i], o[i], h[i], l[i], c[i], barVol, effectiveIB);
      g_SessionEngine.PopulateResult(g_SessionRes);
      ...
      // --- Per-Bar Historical Reconstruction (v7.941 Phase A/Priority 1) ---
      // Bar[i] -> Session[i] (above) -> Profile[i] -> State[i] -> P3[i] -> P4[i] -> P5[i] -> Score[i]
      // Safe to call every bar now: Update()'s developing path uses an incremental M1
      // cache (see CMarketProfileEngine::Update) instead of re-fetching/rescanning the
      // whole session's M1 history on every call, so this no longer causes the
      // freeze/missing-arrow regression. targetTime=t[i] bounds the M1 data used to
      // bar i's own time, so no future information leaks into historical bars.
      g_MPEngine.Update(g_SessionRes, g_ProfileRes, atrFastArr[i], 0, c[i], t[i]);
      g_StateEngine.AnalyzeState(g_ProfileRes, c[i], h[i], l[i], atrFastArr[i], g_SessionRes.ibState, t[i], g_StateRes);
      g_Phase3Engine.EvaluateAcceptanceRejection(i, g_ProfileRes, o, h, l, c, tv, g_Phase3Data);
      g_Phase4Engine.EvaluateSequenceContext(i, g_ProfileRes, g_StateRes, g_Phase3Data, o, h, l, c, tv, g_e2, g_Phase4Data);
      g_Phase5Engine.EvaluatePhase5Confluence(i, g_Phase4Data, g_StateRes, atrFastArr[i], t[i], g_Phase5Data);
      ...
      CheckFilters(i,o,h,l,c,tv, atrFastArr[i]);
      CalcDoP(c[i]);

      UpdateMarketStructureState(i, h, l, c);
      UpdateOrderBlock(i, o, h, l, c);
      UpdateLiquidityMap(i, h, l, t, atrFastArr[i]);

      g_scoreBufBull[i] = CalculateInstitutionalScore(i, o, h, l, c, tv, true,  t[i], atrFastArr[i]);
      g_scoreBufBear[i] = CalculateInstitutionalScore(i, o, h, l, c, tv, false, t[i], atrFastArr[i]);

      CheckSignals(i,o,h,l,c,tv,t, i < (rt - 1), atrFastArr[i]);
   }
...
bool DetectDisplacement(int i, const double &o[], const double &h[], const double &l[], const double &c[], bool isBullish, double atrValue)
{
   ...
   // v7.94-2 review item 12: use the ATR value belonging to bar i (historical) instead
   // of the current-only g_atr_fast, so historical SMC displacement detection isn't
   // contaminated by present-day volatility.
   bool strongBody = (atrValue > 0) ? (body >= atrValue * 1.2) : ((body / range) >= 0.70);
...
{
   if(i < 2) return false;
   // v7.94-2 review item 12: same current-ATR-in-SMC pattern as DetectDisplacement —
   // use atrValue (bar i's own ATR) instead of the current-only g_atr_fast.
   double minGap = (atrValue > 0) ? (atrValue * InpMinFVG_ATR_Ratio) : (_Point * 10);
   bool displacement = DetectDisplacement(i-1, o, h, l, c, isBullish, atrValue);
...
void UpdateLiquidityMap(int i, const double &h[], const double &l[], const datetime &t[], double atrValue)
{
   ...
   // v7.94-2 review item 12: same current-ATR-in-SMC pattern — use atrValue (bar i's
   // own ATR) instead of the current-only g_atr_fast for the equal-high/low tolerance.
   double tol = (atrValue > 0) ? (atrValue * 0.15) : (_Point * 20);
...
double CalculateInstitutionalScore(int i, const double &o[], const double &h[], const double &l[], const double &c[], const long &tv[], bool isBullish, datetime barTime, double atrValue)
{
   ...
   bool disp   = DetectDisplacement(i, o, h, l, c, isBullish, atrValue);
   ...
   bool fvg = DetectInstitutionalFVG(i, o, h, l, c, isBullish, atrValue);
   bool obRetest = false;
   ...
   if(InpUseMTFBiasFilter)
   {
      ...
      {
         // v7.94-2 review item 11: resolve the H1 MTF bar relative to barTime instead of
         // always shift 0 (= current, still-forming H1 bar "right now").
         int mtfShift = iBarShift(_Symbol, g_mtfT[4], barTime);
         if(mtfShift < 0) mtfShift = 0;
         if(CopyBuffer(g_hMTF_Fast[4], 0, mtfShift, 1, e1h_f) > 0 && CopyBuffer(g_hMTF_Slow[4], 0, mtfShift, 1, e1h_s) > 0)
         {
...
void CheckSignals(int i, const double &o[], const double &h[], const double &l[], const double &c[], const long &tv[], const datetime &t[], bool isHistory, double atrValue)
{
   ...
   if(s1l || s1s || s2l || s2s)
   {
      if(s1l)      DrawSig(i,cc,h[i],l[i],1,t,bullScore,isHistory,atrValue);
      else if(s1s) DrawSig(i,cc,h[i],l[i],2,t,bearScore,isHistory,atrValue);
      else if(s2l) DrawSig(i,cc,h[i],l[i],3,t,bullScore,isHistory,atrValue);
      else if(s2s) DrawSig(i,cc,h[i],l[i],4,t,bearScore,isHistory,atrValue);
...
// v7.943: Auto-DST helpers (opt-in via InpAutoDST). Pure date math, no side effects —
// do not read/write any session/profile/state/score data.
// UK British Summer Time:      last Sunday of March 01:00 UTC -> last Sunday of October 01:00 UTC.
// US Eastern Daylight Time:    2nd Sunday of March 07:00 UTC (2am EST) -> 1st Sunday of November 06:00 UTC (2am EDT).
datetime GetNthSundayOfMonth(int year, int month, int n)
{
   MqlDateTime dt;
   ...
   MqlDateTime first;
   TimeToStruct(firstOfMonth, first);
   int daysToFirstSunday = (7 - first.day_of_week) % 7;
   int targetDay = 1 + daysToFirstSunday + (n - 1) * 7;
...
datetime GetLastSundayOfMonth(int year, int month)
{
   ...
   MqlDateTime dt;
   dt.year = nextYear; dt.mon = nextMonth; dt.day = 1; dt.hour = 0; dt.min = 0; dt.sec = 0;
   datetime firstOfNextMonth = StructToTime(dt);
   datetime lastDayOfMonth = firstOfNextMonth - 86400;

   MqlDateTime last;
   TimeToStruct(lastDayOfMonth, last);
   datetime lastSunday = lastDayOfMonth - (datetime)(last.day_of_week * 86400);
   return lastSunday;
}
...
bool IsUKDST(datetime utcTime)
{
   ...
   datetime dstStart = GetLastSundayOfMonth(dt.year, 3)  + 1 * 3600; // 01:00 UTC, last Sun March
   datetime dstEnd   = GetLastSundayOfMonth(dt.year, 10) + 1 * 3600; // 01:00 UTC, last Sun October
...
bool IsUSDST(datetime utcTime)
{
   MqlDateTime dt;
   TimeToStruct(utcTime, dt);
   datetime dstStart = GetNthSundayOfMonth(dt.year, 3, 2)  + 7 * 3600; // 2am EST = 07:00 UTC, 2nd Sun March
   datetime dstEnd   = GetNthSundayOfMonth(dt.year, 11, 1) + 6 * 3600; // 2am EDT = 06:00 UTC, 1st Sun November
   return (utcTime >= dstStart && utcTime < dstEnd);
}
...
void ApplyTFAdaptive()
{
   ...
   if(tf == PERIOD_M1)       { g_ib=30; }
   else if(tf == PERIOD_M5)  { g_ib=60; }
   else if(tf == PERIOD_M15) { g_ib=90; }
   else if(tf == PERIOD_M30) { g_ib=120; }
   else if(tf == PERIOD_H1)  { g_ib=240; }
   else                      { g_ib=60; }
}
...
void CheckFilters(int i,const double &o[],const double &h[],const double &l[],const double &c[],const long &tv[], double atrValue)
{
   ...
   g_bf = ((body / range) * 100.0 >= InpManBodyPct);

   double vs = 0; int cnt = MathMin(g_vsma, i + 1);
   for(int j = 0; j < cnt; j++) vs += (double)tv[i - j];
   vs /= cnt;
   g_vf = (vs > 0) ? ((double)tv[i] >= vs * InpManVolMult) : true;

   // Use the ATR value belonging to bar i (historical) rather than the current-only
   // g_atr_fast, so historical filter evaluation is not contaminated by present ATR (v7.941 Priority 3 Fix)
   g_af = (atrValue > 0) ? (range >= atrValue * 0.5) : true;
   g_all = (g_bf && g_vf && g_af);
}

void CalcDoP(double cp)
{
   if(g_ProfileRes.VAH > 0 && cp > g_ProfileRes.VAH) g_dop = 1;
   else if(g_ProfileRes.VAL > 0 && cp < g_ProfileRes.VAL) g_dop = -1;
   else g_dop = 0;
}

// v7.943: Test 1 visual verification aid. Draws one dashed vertical line per session
// transition, color-coded by session name, with a hover tooltip showing the exact
// transition time and session name — so session boundaries can be checked by eye
// directly on the chart instead of watching the dashboard live. Purely additive/visual:
// does not read or write any session/profile/state/score data, so it cannot affect any
// other calculation. Toggle: InpShowSessionLines.
void DrawSessionBoundary(datetime transitionTime, string sessionName)
{
   if(!InpShowSessionLines) return;

   string name = g_P + "SESBND_" + IntegerToString((long)transitionTime);
   if(ObjectFind(0, name) >= 0) return;

   color lineColor = clrGray;
   if(sessionName == "Asia")           lineColor = clrDodgerBlue;
   else if(sessionName == "London")    lineColor = clrLimeGreen;
   else if(sessionName == "New York")  lineColor = clrOrange;
   else if(sessionName == "Overnight") lineColor = clrSilver;

   if(!ObjectCreate(0, name, OBJ_VLINE, 0, transitionTime, 0)) return;
   ...
   ObjectSetString(0, name, OBJPROP_TOOLTIP, sessionName + " start @ " + TimeToString(transitionTime, TIME_DATE|TIME_MINUTES));
}

void DrawSig(int i, double p, double h, double l, int type, const datetime &t[], double score, bool isHistory, double atrValue)
{
   string name = g_P + "SIG_" + IntegerToString((long)t[i]);
   if(ObjectFind(0, name) >= 0) return;

   bool isBuy = (type == 1 || type == 3);
   // v7.94-2 review item 13: use the ATR that belonged to bar i (already computed by the
   // caller's per-bar atrFastArr[i]) instead of the current-only g_atr_fast, so arrow
   // placement and SL/TP for a historical signal are not derived from today's volatility.
   double arrowPrice = isBuy ? l - (atrValue * 0.3) : h + (atrValue * 0.3);
   if(arrowPrice <= 0) arrowPrice = p;

   if(!ObjectCreate(0, name, OBJ_ARROW, 0, t[i], arrowPrice)) return;

   ObjectSetInteger(0, name, OBJPROP_ARROWCODE, isBuy ? ARROW_BUY : ARROW_SELL);
   ObjectSetInteger(0, name, OBJPROP_COLOR, isBuy ? InpColBuy : InpColSell);
   ObjectSetInteger(0, name, OBJPROP_WIDTH, (type >= 3) ? 3 : 2);
   ObjectSetInteger(0, name, OBJPROP_ANCHOR, isBuy ? ANCHOR_TOP : ANCHOR_BOTTOM);

   double slDist = (atrValue > 0) ? (atrValue * 1.5) : (_Point * 100);
   double slPrice  = isBuy ? (p - slDist) : (p + slDist);
   double tp1Price = isBuy ? (p + slDist * g_Phase5Data.dynamicTp1Multiplier) : (p - slDist * g_Phase5Data.dynamicTp1Multiplier);
   double tp2Price = isBuy ? (p + slDist * g_Phase5Data.dynamicTp2Multiplier) : (p - slDist * g_Phase5Data.dynamicTp2Multiplier);

   string meta = DoubleToString(p, _Digits) + ";" + 
   ...
   {
      string sigName = isBuy ? "BUY SIGNAL" : "SELL SIGNAL";
      TriggerAlert(sigName + " @ " + DoubleToString(p, _Digits) + " (Score: " + DoubleToString(score, 1) + ")");
   }
}
...
void DrawTargetLine(string name, datetime st, datetime et, double price, color clr, int style, int width, string label)
{
   ...
   {
      ObjectSetInteger(0, objName, OBJPROP_TIME, 0, st);
      ObjectSetDouble(0, objName, OBJPROP_PRICE, 0, price);
      ObjectSetInteger(0, objName, OBJPROP_TIME, 1, et);
...
void TriggerAlert(string msg)
{
   string fullMsg = "BTW System (" + _Symbol + "): " + msg;
   if(InpSound) Alert(fullMsg);
   if(InpPush) SendNotification(fullMsg);
}

void SavePrior()
{
   if(g_SessionRes.sessionStart <= 0) return;
...
void DrawIB(datetime ct)
{
   if(!g_SessionRes.ibSet && g_SessionRes.ibHigh == 0) return;
   datetime st = g_SessionRes.sessionStart;
   int effectiveIB = InpAutoTF ? g_ib : InpIBMinutes;
   datetime et = st + effectiveIB * 60;

   DrawLvl("IB_H", et, g_SessionRes.ibHigh, InpColIB, STYLE_SOLID, "IBH " + DoubleToString(g_SessionRes.ibHigh, _Digits));
   DrawLvl("IB_L", et, g_SessionRes.ibLow,  InpColIB, STYLE_SOLID, "IBL " + DoubleToString(g_SessionRes.ibLow, _Digits));
}

void DrawPrior(int rt)
{
   if(g_pri.dt <= 0) return;
   datetime ct = TimeCurrent();
   DrawLvl("PRI_POC", ct, g_pri.poc, InpColPrior, STYLE_DASH, "Prior POC " + DoubleToString(g_pri.poc, _Digits));
   DrawLvl("PRI_VAH", ct, g_pri.vah, InpColPrior, STYLE_DOT,  "Prior VAH " + DoubleToString(g_pri.vah, _Digits));
   DrawLvl("PRI_VAL", ct, g_pri.val, InpColPrior, STYLE_DOT,  "Prior VAL " + DoubleToString(g_pri.val, _Digits));
}
...
void CreateDash()
{
   int x = 200, y = 30, dy = 16;
   MkLbl("D_TITLE", x, y, "=== MTF TREND MATRIX ===", clrGold, 9); y += dy;
   for(int k=0; k<6; k++)
   {
      MkLbl("D_MTF"+IntegerToString(k), x, y, g_mtfN[k] + " : ", clrWhite, 8);
      MkLbl("D_K2"+IntegerToString(k),  x+45, y, "--", clrGray, 8);
      MkLbl("D_E2"+IntegerToString(k),  x+95, y, "--", clrGray, 8);
      y += dy;
   }
}
...
bool Find(string src, string query)
{
   return (StringFind(Lower(src), Lower(query)) >= 0);
}
